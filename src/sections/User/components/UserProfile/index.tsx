...
export const UserProfile = ({user, viewerIsUser, viewer, setViewer, handleUserRefetch}: Props) => {
    const [disconnecteStripe, {loading}] = useMutation<DisconnectStripeData>(
        DISCONNECT_STRIPE, {
            onCompleted: data => {
                if (data && data.disconnectStripe) {
                    setViewer({...viewer, hasWallet: data.disconnectStripe.hasWallet})
                    displaySuccessNotification(
                        "You've successfully diconnected from Stripe!",
    ...
    const additionalDetails = user.hasWallet ? (
        <Fragment>
            ...
            <Button type="primary" className="user-profile__details-cta" loading={loading} onClick={() => disconnecteStripe()}>
                Disconnect Stripe
            </Button>
        ...
        <Fragment>
                ...
                <Paragraph type="secondary">
                    TinyHouse uses <a href="https://stripe.com/en-Us/connected" target="_blank" rel="noopener noreferre noreferrer">
                        Stripe</a>{" "}
