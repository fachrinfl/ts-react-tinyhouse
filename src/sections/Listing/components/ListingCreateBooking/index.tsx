...
export const ListingCreateBooking = ({ viewer, bookingsIndex, host, price, checkInDate, checkOutDate, setCheckInDate, setCheckOutDate, setModalVisible }: Props) => {
    const bookingIndexJSON: BookingsIndex = JSON.parse(bookingsIndex);
    ...
    const dateIsBooked = (currentDate: Moment) => {
        ...
        if (bookingIndexJSON[year] && bookingIndexJSON[year][month]) {
            return Boolean(bookingIndexJSON[year][month][day]);
        } else {
            return false;
        }
    }

    const disabledDate = (currentDate?: any): boolean => {
        if (currentDate) {
    ...
    const checkInputDisabled = !viewer.id || viewerIsHost;
    const checkOutInputDisabled = checkInputDisabled || !checkInDate; 
    const buttonDisabled = checkInputDisabled || !checkInDate || !checkOutDate;
    ...
    if (!viewer.id) {
        ...
        buttonMessage = "The host has disconnected from Stripe and this won't be able to receive payments!";
    }
    ...
    return (
        <div className="listing-booking">
            <Card className="listing-booking__card">
                ...
                <Divider />
                <Button disabled={buttonDisabled} size="large" type="primary" className="listing-booking__card-cta" onClick={() => setModalVisible(true)}>
                    Request to book!
                </Button>
